import os
from zipfile import ZipFile
import urllib.request
import numpy as np
import imageio
from ..utils import download_dataset
from tqdm import tqdm
from sklearn.preprocessing import LabelEncoder

from PIL import Image


_urls = {"tiny-imagenet-200.zip": "http://cs231n.stanford.edu/tiny-imagenet-200.zip"}
_checksums = {"tiny-imagenet-200.zip": "90528d7ca1a48142e341f4ef8d21d0de"}
_name = "tinyimagenet"
SHAPE = (3, 64, 64)
# hashlib.md5(open('../../Downloads/tinyimagenet/tiny-imagenet-200.zip','rb').read()).hexdigest()
N_SAMPLES = 100000


def load(path=None, as_array=False):
    """
    Tiny Imagenet has 200 classes. Each class has 500 training images, 50
    validation images, and 50 test images. We have released the training and
    validation sets with images and annotations. We provide both class labels an
    bounding boxes as annotations; however, you are asked only to predict the
    class label of each image without localizing the objects. The test set is
    released without labels. You can download the whole tiny ImageNet dataset
    here.
    """

    download_dataset(name=_name, name_to_url=_urls, path=path)

    # Loading the file
    f = ZipFile(os.path.join(path, _name, "tiny-imagenet-200.zip"), "r")
    names = [name for name in f.namelist() if name.endswith("JPEG")]
    val_classes = np.loadtxt(
        f.open("tiny-imagenet-200/val/val_annotations.txt"),
        dtype=str,
        delimiter="\t",
    )
    val_classes = dict([(a, b) for a, b in zip(val_classes[:, 0], val_classes[:, 1])])
    x_train, x_test, x_valid, y_train, y_test, y_valid = [], [], [], [], [], []
    for name in tqdm(names, desc=f"Loading {_name}"):
        im = Image.open(f.open(name)).convert("RGB")
        if "train" in name:
            classe = name.split("/")[-1].split("_")[0]
            x_train.append(im)
            y_train.append(classe)
        if "val" in name:
            x_valid.append(im)
            arg = name.split("/")[-1]
            y_valid.append(val_classes[arg])
        if "test" in name:
            x_test.append(im)
    labels = LabelEncoder().fit(y_train)
    dataset = {
        "train": {"X": x_train, "y": labels.transform(y_train)},
        "test": {
            "X": x_test,
            "y": np.ones(len(x_test), dtype="int") * np.nan,
        },
        "val": {"X": x_valid, "y": labels.transform(y_valid)},
    }
    if as_array:
        X = np.empty((len(dataset["train"]["X"]), 64, 64, 3))
        for i in tqdm(range(len(X))):
            X[i] = np.array(dataset["train"]["X"][i])
            dataset["train"]["X"][i] = None
        dataset["train"]["X"] = X
        X = np.empty((len(dataset["val"]["X"]), 64, 64, 3))
        for i in tqdm(range(len(X))):
            X[i] = np.array(dataset["val"]["X"][i])
            dataset["val"]["X"][i] = None
        dataset["val"]["X"] = X

    return dataset
